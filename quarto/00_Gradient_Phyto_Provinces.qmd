...
Out of curiosity $\alpha=0.00$ and `ward.D`
...
dimReduc_obj <- plot_dimReduc_coords(
  ...
  clusters = dfPlots$hclust_a0,weights = df_geo_abiotics$Depth)
...
eco_groups_zoop_tax_lat_depth = dfGrump_longer_filtered %>% 
  mutate(Eco_relevant_plank_groups=ifelse(Eco_relevant_plank_groups=='',
                                          'NotAssigned',Eco_relevant_plank_groups),
...
eco_groups_zoop_tax_lat_depth %>% 
  bind_cols(mat_cluster_membership_label %>% transmute(Cluster = factor(hclust_a0))) %>%
  pivot_longer(-c('SampleID','Cluster'),names_to = 'EcoRel',values_to = 'RA') %>%
  group_by(Cluster,EcoRel) %>% 
  summarise(MeanRA = mean(RA)) %>% 
  ggplot(aes(y=Cluster,fill=EcoRel,x=MeanRA))+
  geom_col()+
  theme_minimal()
```
...
list_plot_all_samples = eco_groups_zoop_tax_lat_depth %>% 
  bind_cols(mat_cluster_membership_label %>% transmute(Cluster = factor(hclust_a0))) %>%
  pivot_longer(-c('SampleID','Cluster'),names_to = 'EcoRel',values_to = 'RA') %>%
  group_split(Cluster) %>% 
  lapply(., function(xx){
    xx %>% left_join(df_geo_abiotics %>% select(SampleID,Depth)) %>% 
      arrange(-Depth) %>% 
      ggplot(aes(y=SampleID,fill=EcoRel,x=RA))+
      geom_col()+
      theme_minimal()
      })

ggpubr::ggarrange(plotlist = list_plot_all_samples,ncol = 1)
```
